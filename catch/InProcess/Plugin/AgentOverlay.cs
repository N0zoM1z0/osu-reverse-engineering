...
namespace LocalCatchAgent.Plugin
{
    internal sealed class AgentOverlay : IDisposable
    {
            ...
            Action<string> logger)
        {
            ...
            result.thread.Name = "LocalCatchAgent.Overlay";
        ...
        private sealed class OverlayForm : Form
        {
            ...
            public OverlayForm(Func<AgentOverlaySnapshot> provider)
            {
                ...
                BackColor = Color.FromArgb(15, 22, 25);
                ClientSize = new Size(430, 76);
                DoubleBuffered = true;
            ...
            protected override void OnPaint(PaintEventArgs eventArgs)
            {
                ...
                graphics.Clear(Color.FromArgb(15, 22, 25));
                Color accent = current.Options.Enabled
                    ? Color.FromArgb(98, 220, 174)
            ...
            private void OnTimerTick(object sender, EventArgs eventArgs)
            {
                try
                {
                    ...
                    int wantedHeight = snapshot.MenuVisible ? 390 : 76;
                    if (ClientSize.Width != 430 || ClientSize.Height != wantedHeight)
                        ClientSize = new Size(430, wantedHeight);
                    NativeRect client;
            ...
            {
                DrawText(graphics, "LOCAL CATCH AGENT", titleFont, Color.White, 18, 12, 245, 20);
                DrawRightText(graphics, current.Options.Enabled ? "AGENT" : "PLAYER",
                    titleFont, accent, 280, 12, 112, 20);
                DrawText(graphics, current.Runtime.Phase + "  " + current.Runtime.Detail,
                    smallFont, Color.FromArgb(188, 204, 203), 18, 39, 374, 18);
                DrawText(graphics, "Ctrl+Alt+F7 settings   Ctrl+Alt+F8 toggle",
                    smallFont, Color.FromArgb(126, 145, 148), 18, 56, 374, 16);
            }

            private void DrawMenu(Graphics graphics, AgentOverlaySnapshot current, Color accent)
            {
                DrawText(graphics, "LOCAL CATCH AGENT", titleFont, Color.White, 18, 12, 245, 20);
                DrawRightText(graphics, current.Options.Enabled ? "AGENT ON" : "YOU PLAY",
                    titleFont, accent, 255, 12, 137, 20);
                DrawText(graphics, "Ctrl+Alt + Up/Down select, Left/Right change",
                    smallFont, Color.FromArgb(145, 164, 165), 18, 36, 374, 18);

                string[] labels = new string[]
                {
                    "CONTROL", "PATH GEOMETRY", "SAFETY FLOOR", "WAYPOINT WANDER",
                    "TRACKING DEADBAND", "TINY DROPLETS", "FATIGUE", "VARIATION"
                };
                string[] values = new string[]
                {
                    current.Options.Enabled ? "AGENT" : "PLAYER / SELF",
                    AgentOptionsSnapshot.StyleName(current.Options.Style),
                    AgentOptionsSnapshot.FormatPixels(current.Options.SafetyMargin),
                    current.Options.WanderPixels + "px  projected into tube",
                    AgentOptionsSnapshot.FormatPixels(current.Options.TrackingDeadband),
                    current.Options.IncludeTinyDropletsAsHardConstraints ? "HARD / REQUIRED" : "SOFT / OPTIONAL",
                    current.Options.FatigueEnabled ? "ON  bounded drift" : "OFF",
                ...
                for (int row = 0; row < labels.Length; row++)
                {
                    ...
                    DrawText(graphics, labels[row], smallFont, Color.FromArgb(137, 158, 160),
                        22, y + 6, 145, 19);
                    DrawRightText(graphics, values[row], bodyFont,
                        row == current.SelectedRow ? Color.White : Color.FromArgb(214, 226, 225),
                        166, y + 4, 226, 21);
                }

                int statusTop = rowTop + labels.Length * rowHeight + 8;
                using (Pen line = new Pen(Color.FromArgb(49, 67, 69)))
                ...
                DrawText(graphics, current.Runtime.Detail, smallFont, Color.FromArgb(188, 204, 203),
                    108, statusTop + 11, 284, 18);
                if (!String.IsNullOrEmpty(current.Runtime.MapName))
                    DrawText(graphics, current.Runtime.MapName, smallFont, Color.FromArgb(126, 145, 148),
                        18, statusTop + 32, 374, 18);
                DrawText(graphics, "hard feasibility first; personality lives inside the viable tube",
                    smallFont, Color.FromArgb(126, 145, 148), 18, ClientSize.Height - 39, 374, 16);
                DrawText(graphics, "Ctrl+Alt+F7 close   Ctrl+Alt+F8 quick toggle",
                    smallFont, Color.FromArgb(126, 145, 148), 18, ClientSize.Height - 22, 374, 16);
            }
